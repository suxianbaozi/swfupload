package {
	...
	public class SWFUpload extends Sprite {
		...
		private var file_index:Array = new Array();
                ...
                //根据文件size和name组成唯一key来保证不同文件夹里重名文件的问题
		private var file_index_sign:Array = new Array();
                private var isGoNext:Array  = new Array();

                //如果上传数据 超时 自动触发success的 时间等待句柄
                private var file_data_wait_handle:Array = new Array();
		...
		private var goNext_Callback:String;
		...
		private var uploadProgress_Callback:String;
                ...
                private var checkBeforeSelect_Callback:String;
                private var encodeSuccess_Callback:String;
		...
		private var buttonStateDisabled:Boolean;
                ...
                //压缩时候 保留的图片宽度
		private var imgMaxWidth:Number;
                private var imgMaxSize:Number;
		...
		public function SWFUpload() {
			...
			this.stage.addChildAt(this.buttonTextField,3);
			...
			this.uploadProgress_Callback     = "SWFUpload.instances[\"" + this.movieName + "\"].uploadProgress";
                        this.encodeProgress_Callback     = "SWFUpload.instances[\"" + this.movieName + "\"].encodeProgress";
                        this.encodeSuccess_Callback     = "SWFUpload.instances[\"" + this.movieName + "\"].encodeSuccess";
                        this.checkBeforeSelect_Callback     = "SWFUpload.instances[\"" + this.movieName + "\"].checkBeforeSelect";
			...
			this.uploadComplete_Callback     = "SWFUpload.instances[\"" + this.movieName + "\"].uploadComplete";
                        this.goNext_Callback       = "SWFUpload.instances[\"" + this.movieName + "\"].goNext";
			...
			this.uploadURL = root.loaderInfo.parameters.uploadURL;
                        this.imgMaxWidth = root.loaderInfo.parameters.imgMaxWidth;
                        this.imgMaxSize = root.loaderInfo.parameters.imgMaxSize;
                        if(!this.imgMaxWidth) {
                            this.imgMaxWidth = 1024;
                        }
                        if(!this.imgMaxSize) {
                            this.imgMaxSize = 2;
                        }
		...
		* *************************************** */
		...
		private function goNext(file_item:FileItem):void {
                ...
                private function FileProgressHttpClient_Handler(file_item:FileItem,bytesLoaded:Number,bytesTotal:Number):void {
                        this.Debug("Event: uploadProgress: File ID: " + file_item.id + ". Bytes: " + bytesLoaded + ". Total: " + bytesTotal);
                        ExternalCall.UploadProgress(this.uploadProgress_Callback, file_item.ToJavaScriptObject(), bytesLoaded, bytesTotal);
                }
		...
		private function Complete_Handler(event:Event):void {
			...
			if (serverDataTimer != null) {
				...
				this.serverDataTimer = null;
                        ...
                        this.Debug(file_item.file_reference.name+"数据传输完毕");
		...
		private function HTTPError_Handler(event:HTTPStatusEvent):void {
			...
			if (isSuccessStatus) {
				...
				this.Debug("Event: uploadError: HTTP ERROR : File ID: " + file_item.id + ". HTTP Status: " + event.status + ".");
				ExternalCall.UploadError(this.uploadError_Callback, this.ERROR_CODE_HTTP_ERROR, file_item.ToJavaScriptObject(), event.status.toString());
				this.UploadComplete(true,file_item); 	// An IO Error is also called so we don't want to complete the upload yet.
			}
		...
		public function IOError_Handler(event:IOErrorEvent):void {
			...
			if (file_item.file_status != FileItem.FILE_STATUS_ERROR) {
				...
				this.Debug("Event: uploadError : IO Error : File ID: " + file_item.id + ". IO Error: " + event.text);
				ExternalCall.UploadError(this.uploadError_Callback, this.ERROR_CODE_IO_ERROR, file_item.ToJavaScriptObject(), event.text);
			}
		...
		private function Select_Handler(file_reference_list:Array):void {
			...
			if (queue_slots_remaining < file_reference_list.length) {
				...
				for (var i:Number = 0; i < file_reference_list.length; i++) {
					...
					this.file_index[file_item.index] = file_item;
                                        this.saveFileToIndex(file_item);
                                        this.isGoNext[file_item.sign_index] = 0;
					...
					if (is_valid_file_reference) {
						...
						var canCompress:Number = this.CheckFileCanCompress(file_item);
						if(canCompress==1) {
							file_item.canCompress = 1;
                                                        file_item.js_object.canCompress = 1;
						}
			...
			ExternalCall.FileDialogComplete(this.fileDialogComplete_Callback, file_reference_list.length, num_files_queued, this.queued_uploads);
                ...
                private function saveFileToIndex(file_item:FileItem):void {
                    var name:String = file_item.file_reference.name;
                    var size:Number = file_item.file_reference.size;
                    var index:String = name+"_"+size; //保证不重复
                    this.file_index_sign[index] = file_item;
                }

                private function getFileItemByFile(fr:FileReference):FileItem {
                    var name:String = fr.name;
                    var size:Number = fr.size;
                    var index:String = name+"_"+size; //保证不重复
                    return this.file_index_sign[index];
                }
		...
		****************************************************************** */
		...
		private function CancelUpload(file_id:String, triggerErrorEvent:Boolean = true):void {
			...
			if (this.current_file_item != null && (this.current_file_item.id == file_id || !file_id)) {
					...
					this.UploadComplete(false,this.current_file_item);
                                        this.goNext(this.current_file_item);
			} else if (file_id) {
			...
			if (!this.buttonStateDisabled) {
				if (this.buttonAction === this.BUTTON_ACTION_SELECT_FILE) {
					...
					this.StartUpload();
                                        ...
                                        if(ExternalCall.checkBeforeSelect(this.checkBeforeSelect_Callback)) {
                                            this.SelectFiles();
                                        }
				}
			}
		...
		*************************************************************** */
		...
		private function ReturnUploadStart(start_upload:Boolean,sign_index:String):void {
                        var file_item:FileItem = this.file_index_sign[sign_index];
			...
			if (start_upload) {
				try {
					...
					file_item.file_reference.addEventListener(Event.OPEN, this.Open_Handler);
                                        if(file_item.canCompress==0) {
					...
					file_item.file_reference.addEventListener(HTTPStatusEvent.HTTP_STATUS, this.HTTPError_Handler);
                                        if(file_item.canCompress==0) {
                                            file_item.file_reference.addEventListener(Event.COMPLETE, this.Complete_Handler);
                                        }
					...
					if (this.uploadURL.length == 0) {
						this.Debug("Event: uploadError : IO Error : File ID: " + file_item.id + ". Upload URL string is empty.");

						// Remove the event handlers
						this.removeFileReferenceEventListeners(file_item);

						file_item.file_status = FileItem.FILE_STATUS_QUEUED;
						this.file_queue.unshift(file_item);
						js_object = file_item.ToJavaScriptObject();
						file_item = null;
						...
						this.Debug("ReturnUploadStart(): File accepted by startUpload event and readied for upload.  Starting upload to " + request.url + " for File ID: " + file_item.id);
						file_item.file_status = FileItem.FILE_STATUS_IN_PROGRESS;
						//如果需要压缩
						if(file_item.canCompress==0) {     
                                                        this.Debug("数据开始传输");
							file_item.file_reference.upload(request, this.filePostName, false);
						} else { //压缩上传
							this.compressImg();
						}
					}
					...
					ExternalCall.UploadError(this.uploadError_Callback, this.ERROR_CODE_UPLOAD_FAILED, file_item.ToJavaScriptObject(), message);
		...
		private function UploadComplete(eligible_for_requeue:Boolean,file:FileItem = null):void {
			var jsFileObj:Object = file.ToJavaScriptObject();
			
			this.removeFileReferenceEventListeners(file);
			if (!eligible_for_requeue || this.requeueOnError == false) {
				file.file_reference = null;
				this.queued_uploads--;
		...
		private function imgLoadHandle(e:Event):void {	
			...
			this.Debug("开始压缩图片。。。。");
                        ...
                        if(bitmap.width>this.imgMaxWidth) {
                            rate = this.imgMaxWidth/bitmap.width;
                        }
			...
			m.scale(rate, rate);
 	                bd.draw( e.target.loader, m );
                        this.compressAsync(bd);
                        this.goNext(this.current_file_item);
		}
		private function compressAsync(bd:BitmapData):void {
                        var encodeRate:Number = 90;
			var jenc:JPGEncoderIMP = new JPGEncoderIMP(encodeRate,this.current_file_item.sign_index); 
		...
		private function compressProgress(e:ProgressEvent):void{
			...
			//this.Debug("压缩进度:"+percent+"%");
                        ExternalCall.EncodeProgress(this.encodeProgress_Callback, file_item.ToJavaScriptObject(),e.bytesLoaded, e.bytesTotal);
		}
			...
			var fileRef:FileReference = file_item.file_reference;
                        ...
                        if (last_dot_index >= 0) {
                       		...
                       		if(extension=="jpg" || extension=="jpeg") {
        	                        if(fileRef.size>(this.imgMaxSize*1024*1024)) {
                	                        return 1;
                        	        }
	                        }
			}
